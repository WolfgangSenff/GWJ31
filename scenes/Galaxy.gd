...
const MaxSpice = 500
var current_spice = 0 setget set_current_spice

onready var spice_meter = $GUI/MarginContainer/Control/SpiceMeter

func set_current_spice(value) -> void:
    current_spice = value
    spice_meter.value = value

func on_spice_picked_up() -> void:
    current_spice += randi() % 20
    
func _ready() -> void:
    Globals.connect("spice_picked_up", self, "on_spice_picked_up")
